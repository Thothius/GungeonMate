...
import 'paired_partners_store.dart';
...
class MultiplayerSession extends ChangeNotifier with WidgetsBindingObserver {
  ...
  /// Pairing mode flag. When true, the session is in device-pairing
  /// mode (one-time exchange of a persistent pairId) rather than a
  /// normal play session. Drives the post-hello handshake to send/
  /// receive [MpPair] instead of snapshots.
  bool _isPairingMode = false;
  bool get isPairingMode => _isPairingMode;

  /// The pairId generated by the pairing initiator (Main in pairing
  /// mode). Sent to the peer via [MpPair] and persisted on both sides
  /// once the [MpPairAck] round-trips.
  String? _pendingPairId;

  /// True after the local side has persisted the pairing. Used to
  /// short-circuit duplicate [MpPair] messages and to decide whether
  /// to emit the pairing-success notification.
  bool _pairPersistedLocally = false;

  /// Notifier that fires once when a pairing completes successfully.
  /// The lobby UI listens to pop the pairing screen and show a success
  /// toast. Single-shot — value resets to false immediately after the
  /// listener reads it.
  final ValueNotifier<bool> pairingCompleteNotifier = ValueNotifier(false);

  /// The partner nickname captured during pairing, for the local
  /// PairedPartner record. Set from the peer's hello userLabel.
  String? _pairingPeerNickname;
  ...
  Future<void> cancel() async {
    ...
    _pinCode = null;
    _mySnapshotSeq = 0;
    // Clear pairing-mode state so a cancelled pairing doesn't leak into
    // the next session.
  ...
  void dispose() {
    ...
    pairingCompleteNotifier.dispose();
  ...
  void _onServiceEvent(MpServiceEvent e) {
    switch (e) {
      ...
      case MpDisconnected():
        ...
        // Pairing mode: a disconnect during pairing is a failed pairing,
        // not a reconnect candidate. The user re-initiates manually.
        if (_isPairingMode) {
          _log('Pairing session disconnected — aborting pairing.');
          _stopHeartbeat();
          _endPairingSession();
          _error = 'Pairing interrupted. The connection dropped before pairing could complete. Please try again.';
  ...
  void _onHello(MpHello h) {
    ...
    // Capture the peer's nickname for the local PairedPartner record
    // during pairing mode — set before any early return so it's always
    // available when _onPair/_onPairAck fire.
    if (_isPairingMode) {
      _pairingPeerNickname = _peerNickname;
      // In pairing mode we skip the normal connected-flow side effects
      // (snapshot broadcast, session persistence, run-state mutation).
      // The Main (initiator) generates + sends the pairId; the Sidekick
      // waits for it. Both sides persist on ack round-trip.
      _status = MpStatus.connected;
      notifyListeners();
      if (_myRole == MpRole.main) {
        _pendingPairId ??= _newPairId();
        _log('Pairing mode: sending pairId $_pendingPairId to peer...');
        unawaited(
          _service
              .sendMessage(
                MpPair(pairId: _pendingPairId!, nickname: _myNickname),
              )
              .catchError((Object e) {
            _log('Pairing: MpPair send failed: $e');
            _fail('Pairing failed: could not send pairing token.');
            unawaited(_service.disconnect());
            return;
          }),
        );
      } else {
        _log('Pairing mode: awaiting MpPair from initiator...');
      }
      return;
    }
  ...
  void _startSearchTimeout() {
    ...
    _searchTimeout = Timer(Duration(milliseconds: timeoutMs), () {
      if (_status == MpStatus.searching) {
        ...
        } else if (_isPairingMode) {
          _fail(
            'No partner found. Make sure both devices have Bluetooth enabled, are close together, and entered the same pairing PIN.',
          );
          unawaited(_service.stopSearching());
        } else {
          _fail(
  ...
  /// Generate a fresh 8-char uppercase hex pairId. Long enough to be
  /// unguessable as a persistent PIN substitute, short enough to never
  /// need manual entry.
  String _newPairId() {
    final a = _rng.nextInt(1 << 16).toRadixString(16).toUpperCase().padLeft(4, '0');
    final b = _rng.nextInt(1 << 16).toRadixString(16).toUpperCase().padLeft(4, '0');
    return '$a$b';
  }

  /// Start a one-time pairing session as Main (initiator). The user
  /// picks their nickname; the partner joins as Sidekick with the same
  /// 4-digit pairing PIN (displayed in the UI). After hello exchange,
  /// we generate + send the persistent pairId via [MpPair]. Both sides
  /// persist it on ack round-trip, then disconnect — the pair is ready
  /// for one-tap [connectPaired] sessions thereafter.
  ///
  /// `pairingPin` is a 4-digit code the user shares out-of-band with
  /// the partner for this one pairing session only. It's used as the
  /// Nearby endpoint-name filter for this single exchange and then
  /// discarded — the pairId replaces it permanently.
  Future<void> startPairing({
    ...
    _log('Initializing PAIRING session as Main, nick=$nickname, pin=$pairingPin');
    final ok = await _service.requestPermissions();
    if (!ok) {
      _status = MpStatus.permissionDenied;
      _error = 'Bluetooth / Nearby permissions are required for pairing.';
      notifyListeners();
      return;
    }
    _isPairingMode = true;
    ...
    notifyListeners();
    _startSearchTimeout();
    final started = await _service.startAdvertising('$nickname#$pairingPin');
    if (!started) {
      _searchTimeout?.cancel();
      await _service.stopSearching();
      _fail('Could not start advertising. Try toggling Bluetooth.');
    } else {
      _log('Pairing advertising started. Awaiting partner on PIN $pairingPin');
    }
  }

  /// Join a one-time pairing session as Sidekick. The user enters the
  /// 4-digit pairing PIN shared by the initiator. After hello exchange,
  /// we wait for the [MpPair] message carrying the persistent pairId,
  /// persist it, send [MpPairAck], and disconnect.
  Future<void> joinPairing({
    required String nickname,
    required String pairingPin,
  }) async {
    if (_busyTransition) return;
    ...
    _log('Initializing PAIRING session as Sidekick, nick=$nickname, pin=$pairingPin');
    final ok = await _service.requestPermissions();
    if (!ok) {
      _status = MpStatus.permissionDenied;
      _error = 'Bluetooth / Nearby permissions are required for pairing.';
    ...
    notifyListeners();
    _startSearchTimeout();
    final started = await _service.startDiscovery(nickname, pinCode: pairingPin);
    if (!started) {
      _searchTimeout?.cancel();
      await _service.stopSearching();
      _fail('Could not start discovery. Try toggling Bluetooth.');
    } else {
      _log('Pairing discovery started. Scanning for initiator on PIN $pairingPin');
    }
  }

  /// Receiver-side handler for [MpPair]. Persists the partner record,
  /// sends [MpPairAck] back, and tears down the pairing session — the
  /// pair is now established and ready for [connectPaired].
  void _onPair(MpPair msg) {
    if (!_isPairingMode) {
      _log('Received MpPair outside pairing mode — ignoring.');
      return;
    }
    if (_pairPersistedLocally) {
      _log('Already persisted this pair — ignoring duplicate MpPair.');
      return;
    }
    final pairId = msg.pairId;
    if (pairId.isEmpty) {
      _fail('Pairing failed: received empty pairId.');
      unawaited(_service.disconnect());
      return;
    }
    final partnerNick = _pairingPeerNickname ?? msg.nickname;
    final partner = PairedPartner(
      pairId: pairId,
      partnerNickname: partnerNick,
      createdAtMs: DateTime.now().millisecondsSinceEpoch,
    );
    unawaited(PairedPartnersStore.upsert(partner).then((_) {
      _pairPersistedLocally = true;
      _log('Pairing: persisted partner "$partnerNick" with pairId $pairId');
      // Ack so the initiator knows it's safe to persist their copy.
      unawaited(
        _service
            .sendMessage(MpPairAck(pairId: pairId))
            .catchError((Object e) {
          _log('Pairing: MpPairAck send failed: $e');
          return;
        }),
      );
      pairingCompleteNotifier.value = true;
      // Tear down the pairing session — the pair is established.
      _endPairingSession();
    }).catchError((Object e) {
      _fail('Pairing failed: could not save partner record.');
      unawaited(_service.disconnect());
    }));
  }

  /// Initiator-side handler for [MpPairAck]. Now safe to persist our
  /// own copy of the pairing, then tear down the pairing session.
  void _onPairAck(MpPairAck msg) {
    if (!_isPairingMode) {
      _log('Received MpPairAck outside pairing mode — ignoring.');
      return;
    }
    if (_pairPersistedLocally) {
      _log('Already persisted this pair — ignoring duplicate MpPairAck.');
      return;
    }
    final pairId = msg.pairId;
    if (pairId.isEmpty || pairId != _pendingPairId) {
      _log('MpPairAck pairId mismatch (got "$pairId", expected "$_pendingPairId") — ignoring.');
      return;
    }
    final partnerNick = _pairingPeerNickname ?? 'Partner';
    final partner = PairedPartner(
      pairId: pairId,
      partnerNickname: partnerNick,
      createdAtMs: DateTime.now().millisecondsSinceEpoch,
    ...
    unawaited(PairedPartnersStore.upsert(partner).then((_) {
      _pairPersistedLocally = true;
      _log('Pairing: initiator persisted partner "$partnerNick" with pairId $pairId');
      pairingCompleteNotifier.value = true;
      _endPairingSession();
    }).catchError((Object e) {
      _fail('Pairing failed: could not save partner record.');
  ...
  /// Tear down the pairing session and return to idle. Called after
  /// both sides have persisted the pair.
  void _endPairingSession() {
    _isPairingMode = false;
    _pendingPairId = null;
    _pairingPeerNickname = null;
    _stopHeartbeat();
  ...
  /// Cancel an in-progress pairing session (user backed out).
  Future<void> cancelPairing() async {
    if (!_isPairingMode) return;
    _endPairingSession();
  }

  /// One-tap connect to a previously-paired partner. Uses the stored
  /// pairId as the PIN — no manual entry needed. The caller picks the
  /// role: Main (host) or Sidekick (join). For a paired partner, the
  /// Sidekick role is no longer forced to Cultist — see elevated
  /// privileges in the connect screen.
  Future<void> connectPaired({
    required PairedPartner partner,
    required MpRole role,
    String? nickname,
    ...
    final nick = (nickname?.isNotEmpty ?? false) ? nickname! : 'Player';
    _log('Connecting to paired partner "${partner.partnerNickname}" as ${role.name} (pairId=${partner.pairId})');
    // Use the pairId as the PIN — both sides filter endpoints by it.
    _pinCode = partner.pairId;
    _isPairingMode = false;
    _pairPersistedLocally = false;
    _pendingPairId = null;
    _pairingPeerNickname = null;
    if (role == MpRole.main) {
      final char = character ??
          ...
          _runProvider.gungeoneerByName('The Marine') ??
